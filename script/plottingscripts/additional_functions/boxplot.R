...
#' @param ... further arguments for \code{\link{use.dw.spell}}
#' 
#' 
#' 
...
spellBoxPlot <- function(x=dw.spell.dry.mes,y=dw.spell.dry.gen,use.dw.spell=FALSE,field="spell_length",station=c("T0010","T0090","T0129"),type=c("P03GPCA","RMRAINGEN"),title="Dry Spell Duration",extract="dry",observationIndex="obs",xlab="type",ylab="dry spell [day]",season=TRUE,...) {
	...
	if (use.dw.spell) { 
		...
		x <- dw.spell(prec_mes,extract=extract,...)
		y <- lapply(X=prec_gen,FUN=dw.spell,extract=extract,...)
