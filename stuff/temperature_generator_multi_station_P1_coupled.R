...
set.seed(seed)
generationP06GPCA_temp <- NULL #ComprehensiveTemperatureGenerator(station=station,Tx_all=TEMPERATURE_MAX,Tn_all=TEMPERATURE_MIN,year_min=year_min,year_max=year_max,p=p_temp,n_GPCA_iteration=n_GPCA_iter,n_GPCA_iteration_residuals=n_GPCA_iteration_residuals,exogen=exogen,exogen_sim=exogen_sim,sample="monthly",mean_climate_Tn=TN_CLIMATE,mean_climate_Tx=TX_CLIMATE,seed=seed)

print("Start First Generation!")

set.seed(seed)
generationP01GPCA_temp <fgh- ComprehensiveTemperatureGenerator(station=station,Tx_all=TEMPERATURE_MAX,Tn_all=TEMPERATURE_MIN,year_min=year_min,year_max=year_max,p=p_test,n_GPCA_iteration=n_GPCA_iter,n_GPCA_iteration_residuals=n_GPCA_iteration_residuals,exogen=exogen,exogen_sim=exogen_sim,sample="monthly",mean_climate_Tn=TN_CLIMATE,mean_climate_Tx=TX_CLIMATE,seed=seed)
...
print(" Start VAR SElection")
VARselect(generationP01GPCA_temp$var@GPCA_data$final_results,lag.max=20)
VARselect(generationP01GPCA_temp_RMRAINGEN$var@GPCA_data$final_results,lag.max=20)


stop(" Stop here: first experiment!!!")
